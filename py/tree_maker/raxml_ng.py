...
class RaxmlNG (maker_base.MakerBase):
    ...
    def prepare(self, state):
        ...
        output_dir = working_dir.joinpath("raxml-ng")
        ...
        #     }
        # general_args = ["-s", self.config["source"], "-w", state["raxml-ng"]["output_dir"], "-m", state["raxml-ng"]["model"], "-e", str(self.config["raxml_model_optimization_precision"]),
        ...
        # state["raxml-ng"]["submitted_tasks"] = state["raxml-ng"]["survived_tasks"] = len(state["raxml-ng"]["run_ids"])
        # args = [(general_args + ["-n", ri, "-p", str(self.random_seed())]) for ri in state["raxml-ng"]["run_ids"]]
        # state["raxml-ng"]["desc"], state["raxml-ng"]["condor_log"] = htcondor.prepare_submission(
        #     program=state["raxml-ng"]["program"],
        #     program_args=args,
        #     priority=-10,
    ...
    def submit(self, state):
        state["raxml-ng"]["cluster"] = htcondor.submit(state["raxml-ng"]["desc"])
        state["raxml-ng"]["started"] = time_m.time()
        state["state"] = "raxml_submitted"

    def wait(self, state):
        job = htcondor.Job(clusters=state["raxml-ng"]["cluster"], condor_log=state["raxml-ng"]["condor_log"])
        status  = job.wait(timeout=self.config["wait_timout"])
        if status == "done":
            state["raxml-ng"]["overall_time"] = time_m.time() - state["raxml-ng"]["started"]
            module_logger.info("RaXML jobs completed in " + RaxmlResult.time_str(state["raxml-ng"]["overall_time"]))
    ...
    def analyse_logs(self, state):
        ...
        completed = [run_id for run_id in state["raxml-ng"]["run_ids"] if Path(state["raxml-ng"]["output_dir"], "RAxML_bestTree." + run_id).exists()]
        if completed:
            best_completed = time_score_from_log(Path(state["raxml-ng"]["output_dir"], "RAxML_log." + run_id) for run_id in completed)
            # module_logger.info('completed: {} best: {}'.format(len(completed), best_completed))
            running_logs = [f for f in (Path(state["raxml-ng"]["output_dir"], "RAxML_log." + run_id) for run_id in state["raxml-ng"]["run_ids"] if run_id not in completed) if f.exists()]
            ...
            if n_to_kill > 0:
                ...
                run_id_to_del = [ri for ri in state["raxml-ng"]["run_ids"] if int(ri.split(".")[-1]) in to_kill]
                # module_logger.info('run_id_to_del {}'.format(run_id_to_del))
                for ri in run_id_to_del:
                    state["raxml-ng"]["run_ids"].remove(ri)
                state["raxml-ng"]["survived_tasks"] -= len(run_id_to_del)
                # module_logger.info('To kill {}: {} run_ids left: {}'.format(n_to_kill, to_kill, state["raxml-ng"]["run_ids"]))
...
class RaxmlNGResults (maker_base.Results):
    ...
    def read(self):
        self.results = sorted((RaxmlNGResult(best_tree) for best_tree in Path(self.state["raxml-ng"]["output_dir"]).glob("RAxML_bestTree.*")), key=operator.attrgetter("score"))
        self.longest_time = max(self.results, key=operator.attrgetter("time")).time if self.results else 0
        self.overall_time = self.state["raxml-ng"]["overall_time"]
        self.submitted_tasks = self.state["raxml-ng"]["submitted_tasks"]
        self.survived_tasks = self.state["raxml-ng"]["survived_tasks"]
        return self
